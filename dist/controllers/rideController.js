...
async function estimateRide(req, res) {
    ...
    try {
        ...
        const geocode = async (address) => {
            const resp = await axios_1.default.get('https://api.mapbox.com/geocoding/v5/mapbox.places/' + encodeURIComponent(address) + '.json', {
        ...
        const directionsResp = await axios_1.default.get('https://api.mapbox.com/directions/v5/mapbox/driving/' +
        ...
        const options = driver_1.DRIVERS
...
async function confirmRide(req, res) {
    ...
    const foundDriver = driver_1.DRIVERS.find(d => d.id === driver.id && d.name === driver.name);
    if (!foundDriver) {
    ...
    try {
        ...
        await db.run(`INSERT INTO rides (customer_id, origin, destination, distance, duration, driver_id, driver_name, value, date)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [customer_id, origin, destination, distance, duration, foundDriver.id, foundDriver.name, value, now]);
        await db.close();
...
async function getRides(req, res) {
    ...
    if (driver_id !== undefined) {
        const foundDriver = driver_1.DRIVERS.find(d => d.id === driver_id);
        if (!foundDriver) {
            return res.status(400).json({
                error_code: 'INVALID_DRIVER',
                error_description: 'Motorista não encontrado.'
